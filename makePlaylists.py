...
for i in range(max(indexes) + 1):
	playlistsIds.append(
	    sp.user_playlist_create(config.username, f"Test {str(i)}", False)["id"])
	playlist = []

	for n, trackId in enumerate(
	    [audioFeature["id"] for audioFeature in audioFeatures]):
		if indexes[n] == i:
...
for i, playlistId in enumerate(playlistsIds):
	...
	print(
	    f"Added {len(playlists[i])} tracks to playlist \"Test {i}\" in {elapsed:.2f}s."
	)
